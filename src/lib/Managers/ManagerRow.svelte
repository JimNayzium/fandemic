...
<div class="manager" style="{retired ? "background-image: url(/retired.png); background-color: var(--ddd)": ""}" on:keypress={() => goto(`/iff/IFF_Manager_Roster?manager=${key}`)} on:click={() => goto(`/iff/IFF_Manager_Roster?manager=${key}`)}>
