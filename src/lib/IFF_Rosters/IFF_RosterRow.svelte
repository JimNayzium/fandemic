<script>
	...
	export let player;
  // export let IFF_rosterPositions;

	const playerSLotClass = player.slot.replace('/', '').replace('SUPER_', 'S-').replace('REC_', 'R-');
	let  playerSlot = player.slot.replace('SUPER_', 'S ').replace('REC_', 'R ');
  const playerName = player.name;

  // console.log("NAME: ", playerName);
  // console.log("SLOT: ", playerSlot);
  // console.log("\n\n\n\n\n\n\n");
...
<Row>
  ...
  <Cell class="playerCell nameCell" colspan={player.name != "Empty" ? (player.poss == "DEF" ? 2 : 1) : 3}>
    {@html player.name}
  </Cell>
</Row>