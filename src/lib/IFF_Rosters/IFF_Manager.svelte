<script>
  ...
  import IFF_Roster from '../IFF_Rosters/IFF_Roster.svelte';
  ...
  import IFF_ManagerFantasyInfo from '../IFF_Rosters/IFF_ManagerFantasyInfo.svelte';

  import ManagerAwards from '../Managers/ManagerAwards.svelte';
  import { onMount } from 'svelte';
  import { getDatesActive, getRosterIDFromManagerID, getTeamNameFromTeamManagers, buildRecord, createIFFRosterSpots} from '$lib/utils/helperFunctions/universalFunctions';
  ...
  // console.log('rosterPositions: ', rosterPositions);
  ...
  rosterID = typeof rosterID !== 'undefined' ? rosterID : manager;
  ...
  const changeManager = (newManager, noscroll = false) => {
    manager = newManager;
    goto(`/iff/IFF_Manager_Roster?manager=${manager}`, {noscroll})
  };
...
<div class="managerContainer">
    <div class="managerConstrained">
      ...
      {:else}
        <div class="alert alert-info p-2">*** Custom Photos Can Be Here ***</div>
      {/if}
        ...
        <div class="record">
            <h3 class="m-2">
              <div class="row my-0 lh-base border border-1 rounded-top my-1">
                <div class="col text-center fw-bold p-0 mx-2 my-0">WINS</div>
                <div class="col text-center fw-bold p-0 mx-2 my-0">LOSSES</div>
                <div class="col text-center fw-bold p-0 mx-2 my-0">TIES</div>
              </div>
              <div class="row my-0 lh-base border border-1 rounded-bottom">

                {#if record && record.record && record.record.wins}
                  <div class="col text-center mx-2 my-0">{record.record.wins}</div>
                {:else}
                  <div class="col text-center mx-2 my-0">0</div>
                {/if}


                {#if record && record.record && record.record.losses}
                  <div class="col text-center mx-2 my-0">{record.record.losses}</div>
                {:else}
                  <div class="col text-center mx-2 my-0">0</div>
                {/if}

                {#if record && record.record && record.record.ties}
                  <div class="col text-center mx-2 my-0">{record.record.ties}</div>
                {:else}
                  <div class="col text-center mx-2 my-0">0</div>
                {/if}
            ...
            <!--
            {#each record.innerRecord as result}
        ...
        <div class="border border-1 border-light rounded-3 my-3">
          
          <h3>I.F.F. Roster Ranking</h3>
          ...
          <div class="m-3 p-2">
            ...
            {#if loading}
              ...
              {:else}              
                <IFF_Roster division="1" expanded={false} {rosterPositions} {roster} {leagueTeamManagers} {players} {startersAndReserve} />
              {/if}
        ...
        {#if viewManager.philosophy}
            ...
            <div class="border border-1 border-light rounded-3">
    ...
    {#if !loading}
        <!-- Favorite player -->
        <IFF_ManagerFantasyInfo {viewManager} {players} />
    {/if}
