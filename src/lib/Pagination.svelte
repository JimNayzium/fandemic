...
{#if total > 0 && totPages > 1 }
    <div class="paginationBar">
        {#if page > 0}
            <Icon class="material-icons button" on:click={() => changePage(page - 1)} on:keydown={() => changePage(page - 1)}>chevron_left</Icon>
        {:else}
        ...
        <div class="numbers">
            {#each pageLabels as pageLabel}
                ...
                {:else}
                    <span class="dest pg" on:click={() => changePage(pageLabel - 1)} on:keydown={() => changePage(pageLabel - 1)}>{pageLabel}</span>
                {/if}
            {/each}
        </div>
        {#if page < totPages - 1}
            <Icon class="material-icons button" on:click={() => changePage(page + 1)} on:keydown={() => changePage(page + 1)}>chevron_right</Icon>
        {:else}
            <span class="placeholder" />
        {/if}
    </div>
