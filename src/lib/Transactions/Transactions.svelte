...
<div class="transactions">
	...
	{:else}
		...
		{#if transactions.waivers.length}
			...
			<p on:click={() => goto("/transactions?show=waiver&query=&page=1")} on:keydown={() => goto("/transactions?show=waiver&query=&page=1")}  class="link">( view more )</p>
		{:else}
		...
		{#if transactions.trades.length}
			...
			<p on:click={() => goto("/transactions?show=trade&query=&page=1")} on:keydown={() => goto("/transactions?show=trade&query=&page=1")} class="link">( view more )</p>
		{:else}
