      ...
      for (const rID in leagueTeamManagers.teamManagersMap[year]) {
        for (const mID of leagueTeamManagers.teamManagersMap[year][rID].managers) {
          if (mID == managerID) {
            ...
            goto(`/iff/IFF_Manager_Roster?manager=${managersIndex}`);
            return;
          }
        }
      }
...
export const buildRecord = (newRoster) => {

  const innerRecord = [];
  // Check to make sure that record exists
  if (newRoster && newRoster.metadata && newRoster.metadata.record) {

    // if (newRoster.hasOwnProperty("metadata") && newRoster.metadata.hasOwnProperty("record")) {
    if (!newRoster.metadata || !newRoster.metadata.record) {
      return innerRecord;
    } else {
      // console.log(newRoster.metadata.record);
    }
    // simplify record
...
export const createIFFRosterSpots = (leageData_rosterPositions) => {
  let QB = 0;
  let RB = 0;
  let WR = 0;
  let TE = 0;
  ...
  leageData_rosterPositions.forEach(position => {
    if (position === 'QB') { QB++ }
    else if (position === 'RB') { RB++ }
    else if (position === 'WR') { WR++ }
    else if (position === 'TE') { TE++ }
    else if (position === 'FLEX') { FLEX++ }
    else if (position === 'SUPER_FLEX') { SUPER_FLEX++ }
    else if (position === 'BN') { BN++ }
  ...
  let IFF_roster_spots = {};

  for (let key in numOfRosterSpots) {
    for (let i = 1; i <= numOfRosterSpots[key]; i++) {
      let index = i > 0 ? `_${i}` : '';
      IFF_roster_spots[`${key}${index}`] = `${key.toUpperCase()}${index}`;
    }
  }

  console.log("IFF_roster_spots: ", IFF_roster_spots);
  


  let finalRoster = {};
  let otherFinalRoster = {};
  
  Object.entries(numOfRosterSpots).forEach(item => {
    ...
    for (let i = 0; i < value; i++) {
      let adj_i = i + 1;
      // loop the number of positions spots and create index_keys for the object 
      let objectKey = `${key}_${adj_i}`;
      finalRoster[objectKey] = null;
      
      // roster_spots.IFF_slot = objectKey;
      // roster_spots[key].push({ objectKey:});
      // otherFinalRoster[key].push(roster_spots);
    }
  });
    


  let return_obj = { finalRoster: finalRoster, otherFinalRoster: null };
  // let return_obj = { finalRoser: finalRoster, otherFinalRoster: otherFinalRoster };
  return return_obj;
};
