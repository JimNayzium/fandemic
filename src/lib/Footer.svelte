...
<footer bind:this={el}>
	...
	<div id="navigation">
		<ul>
			{#each tabs as tab}
				{#if !tab.nest}
					<li><div class="navLink" on:click={() => goto(tab.dest)} on:keydown={() => goto(tab.dest)}>{tab.label}</div></li>
				{:else}
					{#each tab.children as child}
                        <!-- Shouldn't show Managers tab unless managers has been populated -->
				        {#if child.label != "Managers" || managers.length > 0}
                            <li><div class="navLink" on:click={() => goto(child.dest)} on:keydown={() => goto(child.dest)} on:click={() => goto(child.dest)}>{child.label}</div></li>
                        {/if}
					{/each}
				{/if}
			{/each}
