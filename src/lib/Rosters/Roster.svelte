...
<div class="team">
	<DataTable class="teamInner" table$aria-label="Team Name" style="width: {innerWidth * 0.95 > 380 ? 380 : innerWidth * 0.95}px;" >
		<Head> <!-- Team name  -->
			<Row>
				<Cell colspan=4 class="r_{division} clickable">
					<h3 on:click={() => gotoManager({leagueTeamManagers, rosterID: roster.roster_id})} on:keypress={() => gotoManager({leagueTeamManagers, rosterID: roster.roster_id})}>
		...
		<Body>
			...
			{/each}
			<Row class="interactive" on:click={toggleSelected} on:keypress={toggleSelected}>
